...
jinja2_env = Environment(loader=PackageLoader('build'))
...
def url(*args) -> str:
    ...
    return '/'.join(trees) if len(trees) >= 2 else '/'
...
def iter_pages(path: str=None, trees: list=None) -> dict:
    if not trees:
        trees = []
    if not path:
...
def build_article(page: dict) -> str:
