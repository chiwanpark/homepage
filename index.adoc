...
** link:https://mm.xyz[mm] is an audio-only social network service. Users can talk with each other by joining a chat room and follow others to talk privately. I develop personalized recommender systems on whom to follow and which audio chat room to join based on graph-based recommendation models. Several methods such as graph pruning and regularization are applied to the models to improve recommendation quality.
