...
def albumarts_maniadb(query):
  result = requests.get('http://www.maniadb.com/api/search/%s/' % query,
    params={
      'sr': 'album',
      'key': 'example',
      'v': 0.5,
...
def iter_pages(path=None, trees=None):
  ...
  for file_name in os.listdir(path):
    ...
    if os.path.isdir(file_path):
      ...
      yield {'name': os.path.splitext(file_name)[0], 'trees': trees,
             'content': content}
...
def copy_assets():
  shutil.copytree(os.path.join('.', 'assets'),
...
def build_itunes():
  ...
  with BytesIO(result.content) as f:
    ...
    for idx, track in enumerate(itunes_lib['Tracks'].itervalues()):
      ...
      lastPlayed = to_aware(
        track.get('Play Date UTC', datetime.utcfromtimestamp(0)))
      added = to_aware(track['Date Added'])
      albumArtist = track.get('Album Artist', None) or track['Artist']

      if track['Album'] not in albums_dict or albums_dict[track['Album']][
...
def build_watcha():
  ...
  return ({
            ...
            'lastPlayed': to_aware(
              iso8601.parse_date(movie['owner_interest']['updated_at']))
...
def build_media():
  ...
  return templates['media'].render(media_list=media_list,
    today=datetime.now(tz=KST_TIMEZONE))
...
def build_article_index(articles):
  def mapper(article):
    article['month'] = datetime.strptime(article['date'], DATE_FORMAT).strftime(
      MONTH_FORMAT)
    return article
